...
## 判断一棵树是否是bst
- 使用中序遍历，保存访问过的上一个节点的值，判断当前访问的值是否大于上一个结点的值，如果大于，就满足BST的性质
```java
    public static int lastVisited = -2147483648;
    public static boolean isBST(BSTNode node) {
        if (node == null) return true;
        if (!isBST(node.lc)) return false;
        if (lastVisited > node.key) return false;
        lastVisited = node.key;
        return isBST(node.rc);
    }
```
